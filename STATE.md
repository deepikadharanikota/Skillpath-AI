...
- **Docker Compose**: Implemented `docker-compose.yml` to orchestrate 4 interconnected services:
  ...
  - `db`: PostgreSQL 18 container (with updated volume mapping `/var/lib/postgresql` for version 18 compatibility).
...
- **Authlib Integration**: Configured `backend/routers/auth.py` to handle OAuth2 login flows via GitHub.
...
- **Middleware**: Integrated Starlette's `SessionMiddleware` (backed by the `itsdangerous` dependency) into FastAPI to securely manage temporary OAuth state cookies during the redirect flow.
...
- **Frontend Redirects**: Separated the internal Docker network URL (`backend:8000`) from the public-facing URL (`localhost:8000`) in the frontend to ensure the browser accurately follows the GitHub login redirect.
