...
dqn_agent.py
------------
Standard Deep Q-Network (Standard DQN), implemented from scratch in pure NumPy:
  ...
  3. Uniform Experience Replay Buffer:
     ...
     - NO Prioritized Experience Replay (SumTree removed from standard training)
...
class StandardDQNAgent:
    ...
    def replay(self) -> Optional[float]:
        ...
        # 2. Standard DQN Target Calculation
        # Target network evaluates all actions for next states:
        next_q_target = self.target_net.forward(next_states)
        # Directly take the maximum Q-value across all actions: max_a' Q_target(s', a')
        max_next_q = np.max(next_q_target, axis=1)
        # Standard Bellman optimality target equation:
    ...
    def load(self, path: str):
        ...
        if "W3" in online_w and "b3" in online_w:
            # Matches standard DQN architecture
...
# ─────────────────────────────────────────────────────────────────────────
# 4. Standard Configuration & Drop-in Aliases
# ─────────────────────────────────────────────────────────────────────────
