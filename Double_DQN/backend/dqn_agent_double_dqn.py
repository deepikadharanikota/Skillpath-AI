"""
dqn_agent_double_dqn.py
-----------------------
Double Deep Q-Network (Double DQN) implementation in pure NumPy:
  ...
  2. Double DQN Decoupled Target Calculation:
     - Step 1: Online network selects best action:
         a* = argmax_a' Q_online(s', a')
     - Step 2: Target network evaluates that chosen action:
         eval_next_q = Q_target(s', a*)
     - Step 3: Compute Double DQN Bellman optimality target:
  ...
  3. Uniform Experience Replay Buffer:
     ...
     - NO Prioritized Experience Replay (SumTree removed)
...
RL_ALGORITHM = os.getenv("RL_ALGORITHM", "DoubleDQN")
ALGORITHM_VERSION = "DoubleDQN-v1"
...
class StandardQNetwork:
    ...
    def train_on_batch(self, states: np.ndarray, actions: np.ndarray, targets: np.ndarray, huber_delta: float = 1.0) -> float:
        ...
        Backpropagation with standard Huber Loss:
          ...
          targets: (batch,) target Q values computed via Double DQN decoupling
        """
...
class DoubleDQNAgent:
    ...
    Double Deep Q-Network Agent (Double DQN):
      ...
      2. Double DQN Decoupled Target:
           Step 1: a* = argmax_a' Q_online(s', a')   (Online network selects next action)
           Step 2: eval_next_q = Q_target(s', a*)     (Target network evaluates selected action)
           Step 3: y = r + gamma * (1 - done) * eval_next_q
        ...
        One training step using Double DQN:
          ...
          2. Double DQN target calculation:
               Step 1 (Online action selection):
                 best_next_actions = argmax_a' Q_online(s', a')
               Step 2 (Target action evaluation):
                 eval_next_q = Q_target(s', best_next_actions)
               Step 3 (Double DQN Bellman optimality target):
                 y = r + gamma * (1 - done) * eval_next_q
    ...
    def save(self, path: str):
        """Save Double DQN model and metadata."""
        data = {
    ...
    def load(self, path: str):
        """
        Load Double DQN model weights and state.
        ...
        logger.info(f"Loading Double DQN checkpoint: version={version}")

        online_w = data.get("online_weights", {})
        if "W3" in online_w and "b3" in online_w:
            # Matches standard Q-network architecture
            ...
            logger.info(f"Successfully loaded Double DQN weights (step={self._train_steps}, epsilon={self.epsilon:.3f})")
        else:
            logger.warning(
                f"Checkpoint {path} contains incompatible weights. "
                f"Initializing new Double DQN architecture with random weights."
            )


# ─────────────────────────────────────────────────────────────────────────
# 4. Drop-in Aliases and Configuration Output
# ─────────────────────────────────────────────────────────────────────────
DQNAgent = DoubleDQNAgent
StandardDQNAgent = DoubleDQNAgent  # Backward compatibility alias
QNetwork = StandardQNetwork
ReplayBuffer = UniformReplayBuffer


def print_rl_configuration():
    """Prints the clear RL Configuration summary as required by Requirement 28."""
    print("=========================================")
    ...
    print("Double DQN")
    print("")
    print("Online Action Selection:")
    print("ENABLED")
    print("")
    print("Target Action Evaluation:")
    print("ENABLED")
    print("")
