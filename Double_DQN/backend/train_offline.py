...
train_offline.py
-----------------
Runs a full Double Deep Q-Network (Double DQN) training loop against the
simulated learner and saves the trained weights to dqn_weights.pkl and
dqn_weights_double_dqn.pkl, which the FastAPI backend loads on startup.
...
def train(episodes=300, max_steps=12, seed=42, save_path="dqn_weights.pkl"):
    ...
    # Also save designated double dqn weights copy
    ddqn_copy_path = os.path.join(os.path.dirname(save_path) or ".", "dqn_weights_double_dqn.pkl")
    agent.save(ddqn_copy_path)
    print(f"\nSuccessfully saved trained {ALGORITHM_VERSION} weights to {save_path} and {ddqn_copy_path}")
    return agent
